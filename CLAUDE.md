...
- **VOICEVOX engine** on `:50021` — optional, binary at `vendor/macos-x64/run` (~20s boot). `tts.py` auto-falls-back to macOS `say -v Kyoko` when it's down. Voice ids: `vv:<style_id>` / `say:<Name>`.
